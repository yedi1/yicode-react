...
class Backpack extends React.Component {
    constructor (props) {
        ...
        this.state = {
            ...
            // this may be refactorable with the share-the-love logic in SpriteSelectorItem
        ...
        // remove the hacky flag that prevents double adding
...
const getTokenAndUsername = state => {
    ...
    if (state.scratchGui.session?.session?.session?.user) {
        return {
            token: state.scratchGui.session?.session?.session?.user?.token,
            username: state.scratchGui.session?.session?.session?.user?.userName
        };
    }
    // Otherwise try to pull testing params out of the URL, or return nulls
    //  a hack for testing the backpack
