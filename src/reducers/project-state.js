...
//设置作品名称并保存到服务器
const SET_PROJECT_TITLE = 'project/SET_PROJECT_TITLE';

//保存新作品成功后，把服务器返回的新作品ID设置为当前作品ID
const SET_PROJECT_PID = 'project/SET_PROJECT_PID';
...
const reducer = function (state, action) {
    ...
    case SET_PROJECT_PID://修改作品ID
        return {...state, title: action.title};
    case SET_PROJECT_TITLE://修改作品名称
        return {...state, ppid: action.ppid};
    default:
        return state;
    }
};
//设置新保存作品的作品id及作者Id
const setProjectNewId = (projectId) => ({type: SET_PROJECT_NEW_ID, projectId:projectId});
//修改作品名称
const setProjectTitle = title => ({type: SET_PROJECT_TITLE,title: title});

//作品ppid
const setProjectPid = (ppid) => ({type: SET_PROJECT_PID, ppid:ppid});
...
export {
    ...
    setProjectTitle,//设置作品名称
    setProjectPid,//设置作品用户ID
};
